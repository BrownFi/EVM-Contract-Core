...
contract BrownfiV2Pair is IBrownfiV2Pair, BrownfiV2ERC20 {
    ...
    function setK(uint _k) external {
        require(msg.sender == IBrownfiV2Factory(factory).kSetter(), 'BrownfiV2: FORBIDDEN');
        k = _k;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'BrownfiV2: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IBrownfiV2Factory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'BrownfiV2: INSUFFICIENT_LIQUIDITY');
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'BrownfiV2: K');
        }
